...
function getHeroTitle(input: {
  ...
  if (input.state === "below-minimum") {
    return "Не хватает до минимального заказа.";
  }
...
function getHeroSummary(input: {
  ...
  if (!input.serviceResolved) {
    return "Дальше нужно подтвердить адрес и время.";
  }
  ...
  return "Проверьте заказ и переходите к оформлению.";
}
...
function getBottomBandSummary(input: {
  ...
  if (input.lineCount === 0) {
    return "Сначала добавьте хотя бы одну позицию, чтобы перейти к подтверждению.";
  }

  if (input.state === "invalidated") {
    return "Некоторые позиции нужно перепроверить перед подтверждением.";
  }

  if (input.state === "below-minimum") {
    return "Минимальная сумма ещё не достигнута. Добавьте ещё одну позицию.";
  }

  return "Состав готов, можно переходить к подтверждению.";
}
        ...
        {cartView.revalidationIssues.length > 0 ? (
          <section className="entry-section">
            <div className="entry-section__head">
              ...
              <h2>Если цена или доступность изменились, покажем это перед подтверждением.</h2>
