...
function getHeroSummary(input: {
  ...
  if (!input.hasItems) {
    return "Пустое подтверждение выглядит как поломка. Сначала нужна хотя бы одна собранная позиция.";
  }

  if (input.needsBasketRepair) {
    return "Перед последним шагом корзина должна снова совпасть с текущим сервисом и актуальной кухней.";
  }

  if (input.timingBlocked) {
    return "Для The Raki время ко времени — это обещание, а не декоративная настройка. Если оно сломалось, сначала фиксируем его заново.";
  }

  if (input.requiresManualConfirmation) {
    return "Чувствительный маршрут должен звучать как внимание команды, а не как фальшивое автоматическое принятие.";
  }
...
function getBottomBandTitle(input: {
  ...
  if (input.requiresManualConfirmation) {
    return "Заказ уйдёт в ручное подтверждение команды, а не в фальшивый автомат.";
  }
...
function getBottomBandSummary(input: {
  ...
  if (!input.hasItems || input.needsBasketRepair) {
    return "Пока корзина пуста или устарела, любое подтверждение будет звучать неправдиво.";
  }

  if (input.timingBlocked) {
    return "Сайт не должен обещать невозможное окно. Если время потеряло валидность, сначала возвращаемся к сервисному слою.";
  }
  ...
  return "Подтверждаем контакт и отправляем заказ на подтверждение без лишних шагов.";
}
        ...
        {requiresManualConfirmation ? (
          <section className="entry-section">
            <div className="entry-section__head">
              ...
              <h2>Команда должна отдельно подтвердить этот маршрут.</h2>
              <p className="muted">
                Это не поломка. Это спокойный ручной шаг перед финальным обещанием по сервису.
              </p>
            </div>
